...
impl<'a> App<'a> {
        ...
        if event::poll(std::time::Duration::from_millis(50))? {
            let should_recalc = match event::read()?.into() {
                ...
                Input { key: Key::Up, .. } => {
                    self.result_index = match self.result_index {
                        None => None,
                        Some(i) => Some((i + self.result_lines.len() - 1) % self.result_lines.len()),
                    };
                    self.result_state.select(self.result_index);
                    false
                },
                Input { key: Key::Esc, .. } => false,
                Input { key: Key::Enter, .. } | Input { key: Key::Char('m'), ctrl: true, .. } => {
                    if let Some(i) = self.result_index {
                        ...
                        let _ = Command::new("nvim").arg(format!("+{}", lineno)).arg(file).status(); 
                    }
                    false
                },
    ...
    fn ui(&mut self, frame: &mut Frame) {
        ...
        let s = if self.display_hidden { "🗹 Show hidden" } else { "☐ Show hidden" };
        frame.render_widget(
            Paragraph::new(s).block(Self::default_block()),
            top_line[1],
        );
        frame.render_stateful_widget(
            List::new(&*self.result_items).block(Self::default_block()).highlight_symbol("»"),
            main_layout[1],
    ...
    fn execute_rg(&mut self) {
        ...
        self.result_lines = self.raw_result.split("\n").filter(|s| {s.len() > 0}).map(|s| {s.to_string()}).collect();
        self.result_items = self.result_lines.iter().map(|l| {ListItem::new(l.into_text().unwrap())}).collect();
